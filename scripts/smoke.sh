...
PRESETS=$(curl -fsS "$BASE_URL/api/models/presets")
echo "$PRESETS" | grep -q '"active"'

echo "Switching model preset to fast..."
curl -fsS -X POST "$BASE_URL/api/models/presets/active" \
  -H "Content-Type: application/json" \
  -d '{"preset":"fast"}' >/dev/null

echo "Listing workflows..."
WORKFLOWS=$(curl -fsS "$BASE_URL/api/workflows")
...
if echo "$STATUS" | grep -q '"waiting_approval"'; then
  ...
  curl -fsS -X POST "$BASE_URL/api/workflows/run/$RUN_ID/approve" \
    -H "Content-Type: application/json" \
